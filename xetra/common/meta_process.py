...
class MetaProcess():
    ...
    def update_meta_file(extract_date_list: list, meta_key: str, s3_bucket_meta: S3BucketConnector):
        ...
        :param: meta_key -> key of the meta file on the bucket 
        :param: s3_buket_meta -> S3BucketConnector for the bucket with the meta file 
        """
    ...
    def return_date_list(first_date: str, meta_key: str, s3_bucket_meta: S3BucketConnector):
        ...
        returns: 
          ...
          return_date_list: list of all dates from min_date till max_date (cant use .today()
          like in the project cuz no or empty data recently in the dataset)
        """
        ...
        try:
            ...
            # Creating a list of dates from first_date until max_date
            dates = [start + timedelta(days=x) for x in range(0, (max_date - start).days +1)]
            # Creating set of all dates meta file 
            ...
            if dates_missing:
                ...
                # Creating a list of dates from min_date until max_date
            ...
            # No meta file found -> reating a date list from first_date - 1 day until max_date
            return_min_date = first_date
            return_dates = [
                (start + timedelta(days=x)).strftime(MetaProcessFormat.META_DATE_FORMAT.value) \
                for x in range(0, (max_date - start).days + 1)
            ] 
